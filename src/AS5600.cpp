...
//  AS5600Calibrator
//
AS5600Calibrator::AS5600Calibrator()
{
  resetToDefaults();
}


uint16_t AS5600Calibrator::circularDifference(uint16_t start, uint16_t end) const
{
  return (end + AS5600_RAW_RANGE - start) & AS5600_RAW_MASK;
}


void AS5600Calibrator::setCalibration(uint16_t minVal, uint16_t maxVal)
{
  setCalibrationMin(minVal);
  setCalibrationMax(maxVal);
}


void AS5600Calibrator::setCalibrationMin(uint16_t minVal)
{
  _settings.minVal = minVal & AS5600_RAW_MASK;
}


void AS5600Calibrator::setCalibrationMax(uint16_t maxVal)
{
  _settings.maxVal = maxVal & AS5600_RAW_MASK;
}


uint16_t AS5600Calibrator::getCalibrationMin() const
{
  return _settings.minVal;
}


uint16_t AS5600Calibrator::getCalibrationMax() const
{
  return _settings.maxVal;
}


void AS5600Calibrator::setWorkingZeroCentiDegrees(uint16_t workingZero)
{
  if (workingZero > AS5600_CALIBRATION_MAX_CENTIDEGREES)
  {
    workingZero = AS5600_CALIBRATION_MAX_CENTIDEGREES;
  }
  _settings.workingZeroCentiDegrees = workingZero;
}


uint16_t AS5600Calibrator::getWorkingZeroCentiDegrees() const
{
  return _settings.workingZeroCentiDegrees;
}


uint16_t AS5600Calibrator::calibratedRange() const
{
  return circularDifference(_settings.minVal, _settings.maxVal);
}


uint16_t AS5600Calibrator::computeCalibratedAngleCentiDegrees(uint16_t raw) const
{
  uint16_t range = calibratedRange();
  if (range < AS5600_CALIBRATION_MIN_RANGE)
  {
    range = AS5600_CALIBRATION_DEFAULT_RANGE;
  }
  uint16_t diff = circularDifference(_settings.minVal, raw & AS5600_RAW_MASK);
  uint32_t angle = (uint32_t)diff * AS5600_CALIBRATION_MAX_CENTIDEGREES / range;
  if (angle > AS5600_CALIBRATION_MAX_CENTIDEGREES)
  {
    angle = AS5600_CALIBRATION_MAX_CENTIDEGREES;
  }
  return (uint16_t)angle;
}


uint16_t AS5600Calibrator::computeDisplayAngleCentiDegrees(uint16_t raw) const
{
  uint16_t calibrated = computeCalibratedAngleCentiDegrees(raw);
  if (calibrated >= _settings.workingZeroCentiDegrees)
  {
    return calibrated - _settings.workingZeroCentiDegrees;
  }
  return 0;
}


float AS5600Calibrator::computeCalibratedAngleDegrees(uint16_t raw) const
{
  return computeCalibratedAngleCentiDegrees(raw) / 100.0f;
}


float AS5600Calibrator::computeDisplayAngleDegrees(uint16_t raw) const
{
  return computeDisplayAngleCentiDegrees(raw) / 100.0f;
}


void AS5600Calibrator::resetToDefaults()
{
  _settings.signature = AS5600_CALIBRATION_SIGNATURE;
  _settings.minVal = 0;
  _settings.maxVal = AS5600_CALIBRATION_DEFAULT_RANGE;
  _settings.workingZeroCentiDegrees = 0;
}


bool AS5600Calibrator::loadFromEEPROM(int address)
{
  AS5600CalibrationSettings stored;
  EEPROM.get(address, stored);
  if (stored.signature != AS5600_CALIBRATION_SIGNATURE)
  {
    resetToDefaults();
...
bool AS5600Calibrator::saveToEEPROM(int address) const
{
  if (_settings.signature != AS5600_CALIBRATION_SIGNATURE)
  {
    return false;
  }
  EEPROM.put(address, _settings);
  return true;
}


const AS5600CalibrationSettings &AS5600Calibrator::settings() const
{
  return _settings;
}


void AS5600Calibrator::setSettings(const AS5600CalibrationSettings &settings)
{
  _settings = settings;
}
