...
//  calibration helpers
const uint16_t AS5600_RAW_RANGE         = 4096;
const uint16_t AS5600_RAW_MASK          = 0x0FFF;
const uint16_t AS5600_CALIBRATION_MAX_CENTIDEGREES = 18000;
const uint16_t AS5600_CALIBRATION_MIN_RANGE        = 10;
const uint16_t AS5600_CALIBRATION_DEFAULT_RANGE    = 2048;
const uint32_t AS5600_CALIBRATION_SIGNATURE        = 0x4B555431;  // "KUT1"
...
struct AS5600CalibrationSettings
{
  uint32_t signature               = AS5600_CALIBRATION_SIGNATURE;
  uint16_t minVal                  = 0;
  uint16_t maxVal                  = AS5600_CALIBRATION_DEFAULT_RANGE;
  uint16_t workingZeroCentiDegrees = 0;
};


class AS5600Calibrator
{
public:
  AS5600Calibrator();

  uint16_t circularDifference(uint16_t start, uint16_t end) const;

  void     setCalibration(uint16_t minVal, uint16_t maxVal);
  void     setCalibrationMin(uint16_t minVal);
  void     setCalibrationMax(uint16_t maxVal);
  uint16_t getCalibrationMin() const;
  uint16_t getCalibrationMax() const;

  void     setWorkingZeroCentiDegrees(uint16_t workingZero);
  uint16_t getWorkingZeroCentiDegrees() const;

  uint16_t calibratedRange() const;
  uint16_t computeCalibratedAngleCentiDegrees(uint16_t raw) const;
  uint16_t computeDisplayAngleCentiDegrees(uint16_t raw) const;
  float    computeCalibratedAngleDegrees(uint16_t raw) const;
  float    computeDisplayAngleDegrees(uint16_t raw) const;

  void     resetToDefaults();
  bool     loadFromEEPROM(int address = 0);
  bool     saveToEEPROM(int address = 0) const;

  const AS5600CalibrationSettings &settings() const;
  void     setSettings(const AS5600CalibrationSettings &settings);

private:
  AS5600CalibrationSettings _settings;
};
